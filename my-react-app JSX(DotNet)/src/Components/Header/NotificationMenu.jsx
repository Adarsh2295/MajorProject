...
const NotificationMenu=()=>{
    const navigate = useNavigate();
        ...
        useEffect(()=>{
            // Only fetch notifications if user exists and has an id
            if (user && user.id) {
                getNotifications(user.id).then((res)=>{
                    setNotifications(res);
                }).catch((error)=>{
                    console.log('NotificationMenu - Error fetching notifications:', error);
                    // Set empty array on error to prevent crashes
                    setNotifications([]);
                })
            } else {
                console.log('NotificationMenu - No user or user.id found, skipping notifications fetch');
                setNotifications([]);
            }
        }, [user])
        const unread=(index)=>{
