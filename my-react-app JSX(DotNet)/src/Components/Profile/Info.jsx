...
const Info=()=>{
    ...
    const handleSave=async ()=>{
        try {
            ...
            // Debug logging
            console.log('Info - Current profile object:', profile);
            console.log('Info - Form values:', form.getValues());
            console.log('Info - Updated profile before API call:', updatedProfile);
            console.log('Info - Profile ID check:', updatedProfile.id, 'Type:', typeof updatedProfile.id)
            ...
            if (!updatedProfile.id) {
                ...
                console.log('Info - Using fallback ID:', updatedProfile.id, 'Source: user.profileId=', user?.profileId, 'user.id=', user?.id);
            }
            
            // Call API to update profile in backend
            const result = await updateProfile(updatedProfile);
            console.log('Info - Profile updated successfully:', result)

            // Update Redux store with the updated profile
            dispatch(changeProfile(updatedProfile));
            successNotification("Success","Profile Updated Successfully");
        } catch (error) {
            console.error('Info - Error updating profile:', error);
            errorNotification("Error", "Failed to update profile: " + (error.message || 'Unknown error'));
    ...
    return <>
        ...
        </div>
        {
            edit ? <>
                ...
                <div className="text-xl xs-mx:text-base mt-1 flex gap-1 items-center">
                    <IconBriefcase className="h-5 w-5" stroke={1.5}/>{profile.jobTitle} &bull; {profile.company}
                </div>
                <div className="flex gap-1 xs-mx:text-base text-lg text-mine-shaft-400 items-center">
                    <IconMapPin className="h-5 w-5" stroke={1.5}/>{profile.location}
                </div>
                <div className="flex gap-1 xs-mx:text-base text-lg text-mine-shaft-400 items-center">
                    <IconTagStarred className="h-5 w-5" stroke={1.5}/>Experience : &nbsp;{profile.totalExp} Years
                </div>
            </>
        }
    </>
