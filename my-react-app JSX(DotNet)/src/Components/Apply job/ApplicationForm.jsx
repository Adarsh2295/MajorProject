...
const ApplicationForm=()=>{
    ...
    const[submit, setSubmit] = useState(false);
        ...
        const handleSubmit=async()=>{
            ...
            let applicant = {...form.getValues(), applicantId:user.id, jobId:id, resume:resume.split(',')[1]};
            console.log('ApplicationForm - Applicant data being sent:', applicant);
            console.log('ApplicationForm - User ID:', user.id);
            console.log('ApplicationForm - Job ID:', id);
            applyJob(applicant).then((res)=>{
                ...
                setSubmit(false);
                console.log('ApplicationForm - Application error:', error);
                console.log('ApplicationForm - Error response data:', error.response?.data);
                console.log('ApplicationForm - Error status:', error.response?.status);
                const errorMessage = error.response?.data?.errorMessage || error.response?.data?.message || error.response?.data || error.message || 'Failed to submit application';
                errorNotification(
                    "Error",
