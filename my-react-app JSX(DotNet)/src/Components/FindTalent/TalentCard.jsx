...
const TalentCard=(props)=>{
    ...
    const handleOffer=(status)=>{
        ...
        const statusMap = {
            'INTERVIEWING': 2,  // InterviewScheduled
            'OFFERED': 4,       // Hired
            'REJECTED': 3       // Rejected
        };
        ...
        changeAppStatus(props.id, backendStatus, interviewTime).then((res)=>{
            ...
            const errorMessage = error.response?.data?.message || error.response?.data?.errorMessage || error.message || 'Unknown error occurred';
       ...
       <div className="bg-mine-shaft-900 p-4 w-96 flex flex-col gap-3 rounded-xl hover:shadow-[0_0_5px_1px_yellow] !shadow-bright-sun-400 bs-mx:w-[50%] md-mx:w-full">
            ...
            <Modal opened={app} onClose={closeApp} title="Application" centered>
                <div className="flex flex-col gap-4">
                    ...
                    <div>
                        Resume : &emsp; <span onClick={()=>openBase64PDF(props.resumeBase64)} className="text-bright-sun-400 hover:underline cursor-pointer text-right text-sm mb-2">{props.name}</span>
                    </div>
                    <div>
