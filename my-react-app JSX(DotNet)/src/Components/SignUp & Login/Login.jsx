        ...
        if(valid) {
            ...
            loginUser(data).then((res)=>{
                ...
                try {
                    ...
                    // Use email from decoded token, fallback to sub claim
                    const userEmail = decoded.email || decoded.sub;
                    dispatch(setUser({...decoded, email: userEmail}));
                } catch (tokenError) {
                    console.error('Error decoding token:', tokenError);
                    // Still set basic user info from response
                    dispatch(setUser({
                        id: res.userId,
                        email: data.email,
                ...
                const errorMessage = error.response?.data?.Message || error.response?.data?.message || error.message || 'Login failed';
                notifications.show({
                    title: 'Login Failed',
