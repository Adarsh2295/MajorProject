...
const form={
    ...
    password : "",
    confirmPassword : "",
    accountType : 0 // 0 = JobSeeker, 1 = Recruiter
}

const SignUp=()=>{
    const [data, setData] = useState(form);
    const [formError, setFormError] = useState({...form, accountType: ""});
    ...
    const handleChange=(event)=>{
        if(typeof(event)==="string"){
            const accountTypeValue = event === "0" ? 0 : 1; // Convert string to number
            setData({...data, accountType: accountTypeValue});
            return;
        }
    ...
    const handleSubmit=()=>{
        ...
        if(valid===true){
            ...
            registerUser(data).then((res)=>{
                ...
                const errorMessage = error.response?.data?.Message || error.response?.data?.message || error.message || 'Registration failed';
                notifications.show({
    ...
    return<><LoadingOverlay
        ...
        <Radio.Group
            ...
            <Radio className="py-3 px-5 sm-mx:px-3 sm-mx:py-2 border has-[:checked]:bg-bright-sun-400/5 has-[:checked]:border-bright-sun-400 border-mine-shaft-800 rounded-lg" autoContrast value="0" label="Job Seeker" />
            <Radio className="py-3 px-5 sm-mx:px-3 sm-mx:py-2 border has-[:checked]:bg-bright-sun-400/5 has-[:checked]:border-bright-sun-400 border-mine-shaft-800 rounded-lg" autoContrast value="1" label="Recruiter" />
            </div>
            </Group>
        </Radio.Group>
        <Checkbox autoContrast label={<>I accept{''} <Anchor>terms & conditions</Anchor></>}/>
        <Button loading={loading} onClick={handleSubmit} autoContrast variant="filled">Sign Up</Button>
        <div className="mx-auto sm-mx:text-sm xs-mx:text-xs">Have an accout ? <span  className="text-bright-sun-400 hover:underline cursor-pointer sm-mx:text-sm xs-mx:text-xs" onClick={()=>{navigate("/login");setFormError({...form, accountType: ""});setData(form)}}>Login</span></div>
    </div>
    </>
}
